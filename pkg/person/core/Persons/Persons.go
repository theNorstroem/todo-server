...
func (s *service) CreatePerson(ctx context.Context, data *personpb.Person) (id string, err error) {
	id, err = s.repository.CreatePerson(ctx, data)
	if err == nil {
		s.moduleBus.Fire(ctx, "person.Persons.Created", id)
	}
...
func (s *service) UpdatePerson(ctx context.Context, data *personpb.Person, idPrs string) (rec *personpb.Person, err error) {
	rec, err = s.repository.UpdatePerson(ctx, data, idPrs)
	if err == nil {
		s.moduleBus.Fire(ctx, "person.Persons.Updated", rec)
	}
	return rec, err
}
